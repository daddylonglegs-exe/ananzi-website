      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 sm:gap-8 items-center">
          {clients.map((client, index) => (
            <ScrollReveal key={client.name} delay={index * 100}>
              <div className="group">
                <div className="bg-card rounded-lg p-4 sm:p-6 transition-all duration-300 hover:bg-accent/10 hover:scale-105">
                  <div className="aspect-[2/1] flex items-center justify-center">
                    <div className="text-lg sm:text-2xl font-bold text-muted-foreground group-hover:text-accent transition-colors duration-300">
                      {client.name}
                    </div>
                  </div>
                </div>
        ...
        <ScrollReveal delay={400}>
          <div className="mt-12 sm:mt-16">
            <h3 className="text-xl sm:text-2xl font-bold text-center mb-8 sm:mb-12">What Our Clients Say</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
              {testimonials.map((testimonial, index) => (
                <div key={index} className="testimonial-card">
                  <div className="flex items-center mb-4">
                    {[...Array(testimonial.rating)].map((_, i) => (
                      <Star key={i} className="h-4 sm:h-5 w-4 sm:w-5 text-accent fill-current" />
                    ))}
                  </div>
                  <Quote className="h-6 sm:h-8 w-6 sm:w-8 text-accent/50 mb-4" />
                  <p className="text-sm sm:text-base text-muted-foreground mb-6 leading-relaxed italic">"{testimonial.quote}"</p>
                  <div>
        ...
        <ScrollReveal delay={500}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 mt-12 sm:mt-16 pt-12 sm:pt-16 border-t border-border">
            <div className="text-center">
              <div className="text-3xl sm:text-4xl font-black text-accent mb-2">250+</div>
            ...
            <div className="text-center">
              <div className="text-3xl sm:text-4xl font-black text-accent mb-2">15+</div>
